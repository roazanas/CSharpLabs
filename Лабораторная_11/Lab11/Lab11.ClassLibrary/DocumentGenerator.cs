...
namespace Lab11.ClassLibrary.Utils
{
    public static class DocumentGenerator
    {
        ...
        public static Document GenerateRandomDocument()
        {
            ...
            string number = $"<YELLOW>{prefix}<WHITE>-{date.Year}-{random.Next(1000, 10000):D4}";
        ...
        public static Invoice GenerateRandomInvoice()
        {
            ...
            DateTime date = DateTime.Now.AddDays(-random.Next(30));
            string author = authors[random.Next(authors.Length)];
            string number = $"<YELLOW>INV<WHITE>-{date.Year}-{random.Next(1000, 10000)}";

            return new Invoice(amount, quantity, productType, number, date, author);
        }

        private static int lastGeneratedNumber = 1000;
        ...
        public static Receipt GenerateRandomReceipt()
        {
            ...
            lastGeneratedNumber++;
            string number = $"<YELLOW>RCP<WHITE>-{date.Year}-{lastGeneratedNumber}";
        ...
        public static Bill GenerateRandomBill()
        {
            ...
            string author = authors[random.Next(authors.Length)];
            string number = $"<YELLOW>BIL<WHITE>-{date.Year}-{random.Next(1000, 10000)}";
        ...
        {
            string number = ReadFromUser("<WHITE>Введите номер документа: ");
            DateTime date = userInterface?.ReadDate() ?? DateTime.Now;
            string author = ReadFromUser("<WHITE>Введите автора документа: ");
        ...
        public static Invoice CreateInvoiceFromUserInput()
        {
            ...
            string productType = ReadFromUser("<WHITE>Введите тип товара:\n");
            for (int i = 0; i < productTypes.Length; i++)
            {
                userInterface?.WriteColored($"<WHITE>{i + 1}. {productTypes[i]}\n");
            }
        ...
        public static Receipt CreateReceiptFromUserInput()
        {
            ...
            string paymentMethod = ReadFromUser("<WHITE>Введите способ оплаты:\n");
            for (int i = 0; i < paymentMethods.Length; i++)
            {
                userInterface?.WriteColored($"<WHITE>{i + 1}. {paymentMethods[i]}\n");
            }
        ...
        {
            string title = ReadFromUser("<WHITE>Введите заголовок заметки: ");
            string content = ReadFromUser("<WHITE>Введите содержание заметки: ");
